...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className="product-hero"><ImagePanel src={product.heroImage} alt={`${product.name} ${product.type}`} priority /><div className="product-heading"><div><p className="eyebrow">MYCELIA {product.objectNumber}</p><h1>{product.name}</h1></div><p>{product.type}</p></div></section>
    <section className="metadata"><div><span>Collection</span>Mycelia</div><div><span>Object</span>{product.objectNumber}</div><div><span>Type</span>{product.type}</div><div><span>Designed</span>Costa Rica</div></section>
    <section className="editorial-split"><div><p className="eyebrow">Story — Natural reference</p><h2>{product.inspiration}</h2></div><div className="body"><p>{product.description}</p><p>The {product.designPrinciples[0].toLowerCase()} of the organism establishes the primary form. Its {product.designPrinciples[1].toLowerCase()} becomes structure; its material character informs surface and colour.</p></div></section>
