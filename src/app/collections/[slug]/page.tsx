...
export default async function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    {collection.status === "forthcoming" ? <InquiryCTA /> : <section className="section"><div className="future-head"><h2>Collection notes.</h2><p>{collection.principles.join(" / ")}</p></div><div style={{ position:"relative", aspectRatio:"16/8", marginTop:"3rem" }}><Image src={collection.gallery[1]} alt={`${collection.name} collection detail`} fill sizes="94vw" style={{ objectFit:"cover" }} /></div></section>}
  </>;
}
